 ...
 * + PRINTS MELHORADOS com informação completa
 * 
 * LÓGICA:
...
#define DISTANCIA_ENTRE_VERTICES 30.0

// Coordenada base
#define LAT_BASE 38.690786
#define LON_BASE -9.295104
...
void setup() {
  ...
  double raio = DISTANCIA_ENTRE_VERTICES / (2.0 * sin(PI / NUM_VERTICES));
  
  for (int i = 0; i < NUM_VERTICES; i++) {
    double angulo = 90.0 - (i * 360.0 / NUM_VERTICES);
    double angulo_rad = (90.0 - angulo) * PI / 180.0;
    
    double delta_x = raio * cos(angulo_rad);
    double delta_y = raio * sin(angulo_rad);
    
    double delta_lat = delta_y / 6371000.0 * (180.0 / PI);
    double delta_lon = delta_x / (6371000.0 * cos(LAT_BASE * PI / 180.0)) * (180.0 / PI);
    
    vertices_lat[i] = LAT_BASE + delta_lat;
  }
  ...
  Serial.print("Distância entre vértices: ");
  Serial.print(DISTANCIA_ENTRE_VERTICES, 1);
  ...
  for (int i = 0; i < NUM_VERTICES; i++) {
    ...
    // Calcular distância até ao próximo vértice
...
void loop() {
  ...
  if (modo == MODO_AUTOMATICO && gps.location.isValid()) {
    ...
    // ========================================
      ...
      if (diff > 0) {
        // Rodar para a esquerda (motor A mais rápido)
        pwm_motor_a = PWM_CENTRO + VELOCIDADE_VIRAR;
        pwm_motor_b = PWM_CENTRO - VELOCIDADE_VIRAR;
      } else {
        // Rodar para a direita (motor B mais rápido)
        pwm_motor_a = PWM_CENTRO - VELOCIDADE_VIRAR;
        pwm_motor_b = PWM_CENTRO + VELOCIDADE_VIRAR;
      }
